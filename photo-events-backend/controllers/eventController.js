...
exports.createEvent = asyncHandler(async (req, res, next) => {
  ...
  // ✅ FIX: Use 'organizer' instead of 'userId'
  const event = await Event.create({
    organizer: req.user._id,  // ✅ CHANGED FROM userId
    name,
    date,
    description,
    location,
  ...
  if (req.user.incrementEventUsage) {
...
exports.getEvents = asyncHandler(async (req, res, next) => {
  ...
  // ✅ FIX: Query by 'organizer' instead of 'userId'
  const query = { organizer: req.user._id };  // ✅ CHANGED FROM userId
...
exports.getEventStats = asyncHandler(async (req, res, next) => {
  ...
  const stats = {
    ...
    registrationProgress: event.expectedGuests 
      ? Math.round((event.registrationCount / event.expectedGuests) * 100) 
      : 0,
    photosUploaded: event.photosUploaded || 0,
    storageUsed: (event.storageUsedMB || 0) + ' MB',
